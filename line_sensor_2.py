...
def talker():
    global Angle, speed, barcode_data_line_QR, text_0, text_1
    ...
    while not rospy.is_shutdown():
        ...
        if barcode_data_product_QR != "QR_X" :
            if barcode_data_product_QR != barcode_data_line_QR :
                if theta != -50:
                    if theta <  -45                : Angle = 1
                    if theta >= -45 and theta < -40: Angle = 0.9
                    if theta >= -40 and theta < -35: Angle = 0.8
                    if theta >= -35 and theta < -30: Angle = 0.7
                    if theta >= -30 and theta < -25: Angle = 0.5
                    if theta >= -25 and theta < -20: Angle = 0.3
                    if theta >= -20 and theta < -15: Angle = 0.2
                    if theta >= -15 and theta < -10: Angle = 0.1
                    if theta >= -10 and theta <  10: Angle = 0
                    if theta >=  10 and theta <  15: Angle = -0.1
                    if theta >=  15 and theta <  20: Angle = -0.2
                    if theta >=  20 and theta <  25: Angle = -0.3
                    if theta >=  25 and theta <  30: Angle = -0.5
                    if theta >=  30 and theta <  35: Angle = -0.7
                    if theta >=  35 and theta <  40: Angle = -0.8
                    if theta >=  40 and theta <  45: Angle = -0.9
                    if theta >=  45                : Angle = -1
                    speed = 0.4
                else : speed = 0
            else : speed = 0
