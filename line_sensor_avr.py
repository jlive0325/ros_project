...
def talker():
    global Angle, speed, barcode_data_line_QR, text_0, text_1, avr_x
    ...
    while not rospy.is_shutdown():
        ...
        if barcode_data_product_QR != "QR_X" :                      ## QR코드에 올라온 물건이 있을때
            if barcode_data_product_QR != barcode_data_line_QR :    ## QR코드에 올라온 물건과 QR라인이 동일하지 않을때
                if theta != -50                    :
                    if theta <  -40                : Angle = 0.3
                    if theta >= -40 and theta < -30: Angle = 0.2
                    if theta >= -30 and theta < -20: Angle = 0.1
                    if theta >= -20 and theta < -10: Angle = 0
                    if theta >= -10 and theta <  10: Angle = 0
                    if theta >=  10 and theta <  20: Angle = -0
                    if theta >=  20 and theta <  30: Angle = -0.1
                    if theta >=  30 and theta <  40: Angle = -0.2
                    if theta >=  40                : Angle = -0.3
                    if theta <  -40                : speed = 0.15
                    if theta >= -40 and theta < -30: speed = 0.15
                    if theta >= -30 and theta < -20: speed = 0.15
                    if theta >= -20 and theta < -10: speed = 0.2
                    if theta >= -10 and theta <  10: speed = 0.25
                    if theta >=  10 and theta <  20: speed = 0.2
                    if theta >=  20 and theta <  30: speed = 0.15
                    if theta >=  30 and theta <  40: speed = 0.15
                    if theta >=  40                : speed = 0.15
                if theta == -50:
                    speed = 0
                    Angle = -0.5
            if barcode_data_product_QR == barcode_data_line_QR :
                speed = 0       
                Angle = 0                                 ## QR코드에 올라온 물건과 QR라인이 동일할때
